...
$(document).ready(function() {
	...
	$(regButton).on('click', function() {
		...
		setTimeout(function() {
			window.open('https://www.ecstaticliving.com/' + window.location.pathname.substr(1) + '?source=SDI#registration', '_blank')
		}, 3000)
		return false
	...
	$(`${regContinue}, ${regContinueMobile}, ${regPayNow}, ${regPayNowMobile}`).on('click', function() {
		...
		var handler = StripeCheckout.configure({
			...
			token: function(token) {
				...
				$.ajax({
					type: 'GET',
					url: 'https://ecstaticliving.institute/stripe_charge.php',
					crossDomain: true,
					dataType: 'jsonp',
					jsonp: 'callback',
